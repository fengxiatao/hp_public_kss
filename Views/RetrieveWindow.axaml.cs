...
namespace FaceLocker.Views
{
    ...
    /// 取出物品窗口
    /// </summary>
    public partial class RetrieveWindow : ReactiveWindow<RetrieveWindowViewModel>
    {
        #region 私有字段
        private readonly ILogger<RetrieveWindow> _logger;
        ...
        {
            _logger = App.GetService<ILogger<RetrieveWindow>>();
            _appStateService = App.GetService<IAppStateService>();

            _logger.LogInformation("RetrieveWindow：开始创建取出物品窗口");

            try
            {
                // 创建ViewModel（通过DI容器）
                ViewModel = new RetrieveWindowViewModel(
                    App.GetService<ILogger<RetrieveWindowViewModel>>(),
                ...
                {
                    _logger.LogInformation("RetrieveWindow WhenActivated 开始设置绑定");

                    // 注册窗口关闭时的清理
                    disposables.Add(Disposable.Create(() =>
                    {
                        _logger.LogDebug("RetrieveWindow 激活状态结束，执行清理");
                        CleanupResources();
                    }));

                    _logger.LogInformation("RetrieveWindow 绑定设置完成");
                });

                _isInitialized = true;
                _logger.LogInformation("RetrieveWindow：取出物品窗口初始化完成");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "RetrieveWindow 初始化过程中发生异常");
                throw;
            }
        }
        #endregion
        ...
        private void InitializeComponent()
        {
            try
            {
                ...
                _logger.LogDebug("RetrieveWindow XAML 加载完成");
            ...
            {
                _logger.LogError(ex, "RetrieveWindow XAML 加载失败");
                throw;
            }
        }
        ...
        {
            base.OnOpened(e);
            _logger.LogInformation("RetrieveWindow：取出物品窗口已打开");
            ...
            try
            {
                ...
                _logger.LogInformation("RetrieveWindow：窗口打开流程完成");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "RetrieveWindow：打开窗口时发生异常");
            }
        }
        ...
        private async Task InitializeCameraAndRecognitionAsync()
        {
            ...
            {
                _logger.LogWarning("RetrieveWindow：窗口未初始化或ViewModel为空，跳过摄像头初始化");
                return;
            }

            try
            {
                _logger.LogInformation("RetrieveWindow：开始初始化摄像头和识别流程");

                // 使用ViewModel初始化摄像头和人脸识别
                await ViewModel.InitializeWindowAsync();

                _logger.LogInformation("RetrieveWindow：摄像头和识别流程初始化完成");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "RetrieveWindow：初始化摄像头和识别流程时发生异常");
            }
        }
        ...
        protected override void OnClosing(WindowClosingEventArgs e)
        {
            try
            {
                _logger.LogInformation("RetrieveWindow：处理窗口关闭请求");
        ...
        {
            _logger.LogInformation("RetrieveWindow：取出物品窗口已关闭");
            ...
            try
            {
                ...
                _logger.LogInformation("RetrieveWindow：窗口状态重置完成");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "RetrieveWindow：关闭窗口时发生异常");
            }
            finally
            {
                try
        ...
        {
            base.OnGotFocus(e);
            _logger.LogDebug("RetrieveWindow：窗口获得焦点");
        ...
        {
            base.OnLostFocus(e);
            _logger.LogDebug("RetrieveWindow：窗口失去焦点");
                ...
                if (!_isForceClosingByAppState)
                {
                    _ = Dispatcher.UIThread.InvokeAsync(() =>
                    {
                        try
                        {
                            this.Activate();
                            this.Focus();
                            _logger.LogDebug("RetrieveWindow：已重新激活窗口焦点");
                        }
                        catch (Exception ex)
                        {
        ...
        #region 公共方法 - 由AppStateService调用
        /// <summary>
        ...
        public void ForceCloseByAppState()
        {
            try
            {
                _logger.LogInformation("AppStateService强制关闭取出物品窗口");
        ...
        private void CleanupResources()
        {
            try
            {
                _logger.LogInformation("开始清理RetrieveWindow资源");
                ...
                _logger.LogInformation("RetrieveWindow资源清理完成");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "清理RetrieveWindow资源时发生异常");
            }
        }
        #endregion
    }
